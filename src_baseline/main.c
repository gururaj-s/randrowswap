    ...
    while (!expt_done)
    {
        ...
        for (numc = 0; numc < NUMCORES; numc++)
        {
            ...
            { /* Try to fetch if EOF has not been encountered. */
                num_fetch = 0;
                while ((num_fetch < MAX_FETCH) && (ROB[numc].inflight != ROBSIZE) && (!writeqfull) && (!readqfull) && (loadq[numc] < 72) && (storeq[numc] < 44))
                {
                    ...
                    { /* Done consuming non-memory-ops.  Must now consume the memory rd or wr. */
                        if (opertype[numc] == 'R')
                        {
                            ...
                            int L3Hit = isHit(L3Cache, addr[numc], false); //addr[numc] is cl address
                        ...
                        { /* This must be a 'W'.  We are confirming that while reading the trace. */
                            if (opertype[numc] == 'W')
                            {
                                ...
                                int L3Hit = isHit(L3Cache, addr[numc], true);
        ...
        if (temp_inst_cntr > 10000000)
        {
            ...
            {
                printf(" - %lld00 Million Instructions\n", temp_inst_cntr2 / 10);
            }
        }
    }
