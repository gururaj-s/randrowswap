...
extern int NUMCORES;
unsigned long long int delay_stall[MAX_NUM_CHANNELS];
...
unsigned int * ract_real[MAX_NUM_CHANNELS][MAX_NUM_RANKS][MAX_NUM_BANKS];
unsigned int * ract_temp[MAX_NUM_CHANNELS][MAX_NUM_RANKS][MAX_NUM_BANKS];
    ...
    for (int i = 0; i < MAX_NUM_CHANNELS; i++){
        ...
        for (int j = 0; j < MAX_NUM_RANKS; j++){
            for (int k = 0; k < MAX_NUM_BANKS; k++){
                ...
                bank_count[i][j][k] = 0;
                b_counter[i][j][k] = 0;
                ract_real[i][j][k] = (unsigned int*)calloc(ROWMEM, sizeof(unsigned int));
                ract_temp[i][j][k] = (unsigned int*)calloc(ROWMEM, sizeof(unsigned int));
            }
        }
