...
request_t *insert_write(long long int physical_address, long long int arrival_time, int thread_id, int instruction_id)
{
    ...
    request_t *new_node = init_new_node(physical_address, arrival_time, this_op, thread_id, instruction_id, 0, 0, 0, 0, 0);
...
int issue_refresh_command_mod(int channel, int rank)
{
    ...
    else
    {
        ...
        if (dram_state[channel][rank][0].state == PRECHARGE_POWER_DOWN_SLOW)
        {
            for (int b = 0; b < NUM_BANKS; b++)
            {
                dram_state[channel][rank][b].next_act = max(cycle + T_XP_DLL + T_RFCMOD, dram_state[channel][rank][b].next_act);
                dram_state[channel][rank][b].next_pre = max(cycle + T_XP_DLL + T_RFCMOD, dram_state[channel][rank][b].next_pre);
                dram_state[channel][rank][b].next_refresh = max(cycle + T_XP_DLL + T_RFCMOD, dram_state[channel][rank][b].next_refresh);
                dram_state[channel][rank][b].next_powerdown = max(cycle + T_XP_DLL + T_RFCMOD, dram_state[channel][rank][b].next_powerdown);
            }
        }
        else if (dram_state[channel][rank][0].state == PRECHARGE_POWER_DOWN_FAST)
        {
            for (int b = 0; b < NUM_BANKS; b++)
            {
                dram_state[channel][rank][b].next_act = max(cycle + T_XP + T_RFCMOD, dram_state[channel][rank][b].next_act);
                dram_state[channel][rank][b].next_pre = max(cycle + T_XP + T_RFCMOD, dram_state[channel][rank][b].next_pre);
                dram_state[channel][rank][b].next_refresh = max(cycle + T_XP + T_RFCMOD, dram_state[channel][rank][b].next_refresh);
                dram_state[channel][rank][b].next_powerdown = max(cycle + T_XP + T_RFCMOD, dram_state[channel][rank][b].next_powerdown);
            }
        }
        else if (dram_state[channel][rank][0].state == ACTIVE_POWER_DOWN)
        {
            for (int b = 0; b < NUM_BANKS; b++)
            {
                dram_state[channel][rank][b].next_act = max(cycle + T_XP + T_RP + T_RFCMOD, dram_state[channel][rank][b].next_act);
                dram_state[channel][rank][b].next_pre = max(cycle + T_XP + T_RP + T_RFCMOD, dram_state[channel][rank][b].next_pre);
                dram_state[channel][rank][b].next_refresh = max(cycle + T_XP + T_RP + T_RFCMOD, dram_state[channel][rank][b].next_refresh);
                dram_state[channel][rank][b].next_powerdown = max(cycle + T_XP + T_RP + T_RFCMOD, dram_state[channel][rank][b].next_powerdown);
            }
        }
        else // rank powered up
        {
            ...
            if (flag) // at least a single bank is open
            {
                for (int b = 0; b < NUM_BANKS; b++)
                {
                    dram_state[channel][rank][b].next_act = max(cycle + T_RP + T_RFCMOD, dram_state[channel][rank][b].next_act);
                    dram_state[channel][rank][b].next_pre = max(cycle + T_RP + T_RFCMOD, dram_state[channel][rank][b].next_pre);
                    dram_state[channel][rank][b].next_refresh = max(cycle + T_RP + T_RFCMOD, dram_state[channel][rank][b].next_refresh);
                    dram_state[channel][rank][b].next_powerdown = max(cycle + T_RP + T_RFCMOD, dram_state[channel][rank][b].next_powerdown);
                }
            }
            else // everything precharged
            {
                for (int b = 0; b < NUM_BANKS; b++)
                {
                    dram_state[channel][rank][b].next_act = max(cycle + T_RFCMOD, dram_state[channel][rank][b].next_act);
                    dram_state[channel][rank][b].next_pre = max(cycle + T_RFCMOD, dram_state[channel][rank][b].next_pre);
                    dram_state[channel][rank][b].next_refresh = max(cycle + T_RFCMOD, dram_state[channel][rank][b].next_refresh);
                    dram_state[channel][rank][b].next_powerdown = max(cycle + T_RFCMOD, dram_state[channel][rank][b].next_powerdown);
                }
            }
        }
        for (int b = 0; b < NUM_BANKS; b++)
